...
interface LibraryEntryDao {
    ...
    companion object {
        /** Order by status orderId (see [io.github.drumber.kitsune.domain.model.library.Status]) and update time */
    ...
    fun getAllLibraryEntry(status: List<LibraryStatus>): PagingSource<Int, LibraryEntry>

    @Query("SELECT * FROM library_table WHERE status IN (:status) AND anime_id IS NOT NULL $ORDER_BY_STATUS")
    fun getAnimeLibraryEntry(status: List<LibraryStatus>): PagingSource<Int, LibraryEntry>

    @Query("SELECT * FROM library_table WHERE status IN (:status) AND manga_id IS NOT NULL $ORDER_BY_STATUS")
    fun getMangaLibraryEntry(status: List<LibraryStatus>): PagingSource<Int, LibraryEntry>
    ...
    @Query("SELECT * FROM library_table WHERE status IN (:status) $ORDER_BY_STATUS")
    suspend fun getAllLibraryEntryByStatus(status: List<LibraryStatus>): List<LibraryEntry>

    @Query("SELECT * FROM library_table WHERE status IN (:status) AND anime_id IS NOT NULL $ORDER_BY_STATUS")
    suspend fun getAnimeLibraryEntryByStatus(status: List<LibraryStatus>): List<LibraryEntry>

    @Query("SELECT * FROM library_table WHERE status IN (:status) AND manga_id IS NOT NULL $ORDER_BY_STATUS")
    suspend fun getMangaLibraryEntryByStatus(status: List<LibraryStatus>): List<LibraryEntry>
