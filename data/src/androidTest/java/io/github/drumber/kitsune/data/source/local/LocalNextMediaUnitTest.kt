...
class LocalNextMediaUnitTest {
    ...
    fun shouldInsertWhenLibraryEntryIsPresent() = runTest {
        ...
        ViewMatchers.assertThat(getNextMediaUnit, `is`(equalTo(localNextMediaUnit)))
    }
    ...
    fun shouldCascadeDelete() = runTest {
        ...
        // then
        val getNextMediaUnit = nextMediaUnitDao.getSingle(localNextMediaUnit.unitId)
        ViewMatchers.assertThat(getNextMediaUnit, `is`(nullValue()))
    }
    ...
    fun shouldGetLibraryEntryWithNextMediaUnit() = runTest {
        ...
        // then
        ViewMatchers.assertThat(libraryEntryWithNextMediaUnit, `is`(notNullValue()))
        ViewMatchers.assertThat(
            libraryEntryWithNextMediaUnit?.libraryEntry,
            `is`(equalTo(localLibraryEntry))
        )
        ViewMatchers.assertThat(
            libraryEntryWithNextMediaUnit?.nextMediaUnit,
            `is`(equalTo(localNextMediaUnit))
        )
    }
    ...
    fun shouldSortLibraryEntries() = runTest {
        ...
        val getIds: (suspend (LibraryFilterOptions.SortBy, direction: LibraryFilterOptions.SortDirection) -> List<String>) =
            { sortBy, direction ->
        ...
        val getTestInstance: (suspend (LibraryFilterOptions.SortBy, List<String>) -> Triple<List<String>, List<String>, List<String>>) =
            { sortBy, expectedAsc ->
                Triple(
                    getIds(sortBy, LibraryFilterOptions.SortDirection.ASC),
                    getIds(sortBy, LibraryFilterOptions.SortDirection.DESC),
                    expectedAsc
                )
            }
        ...
        // when
        val byStatus =
            getTestInstance(LibraryFilterOptions.SortBy.STATUS, listOf("le0", "le1", "le2", "le3"))
        val byStartedAt = getTestInstance(
            LibraryFilterOptions.SortBy.STARTED_AT,
            listOf("le3", "le2", "le1", "le0")
        )
        val byUpdatedAt = getTestInstance(
            LibraryFilterOptions.SortBy.UPDATED_AT,
            listOf("le0", "le1", "le2", "le3")
        )
        val byProgress = getTestInstance(
            LibraryFilterOptions.SortBy.PROGRESS,
            listOf("le2", "le1", "le3", "le0")
        )
        val byRating =
            getTestInstance(LibraryFilterOptions.SortBy.RATING, listOf("le1", "le0", "le3", "le2"))

        // then
        ViewMatchers.assertThat(byStatus.first, equalTo(byStatus.third))
        ViewMatchers.assertThat(byStatus.second, equalTo(byStatus.third.reversed()))
        ViewMatchers.assertThat(byStartedAt.first, equalTo(byStartedAt.third))
        ViewMatchers.assertThat(byStartedAt.second, equalTo(byStartedAt.third.reversed()))
        ViewMatchers.assertThat(byUpdatedAt.first, equalTo(byUpdatedAt.third))
        ViewMatchers.assertThat(byUpdatedAt.second, equalTo(byUpdatedAt.third.reversed()))
        ViewMatchers.assertThat(byProgress.first, equalTo(byProgress.third))
        ViewMatchers.assertThat(byProgress.second, equalTo(byProgress.third.reversed()))
        ViewMatchers.assertThat(byRating.first, equalTo(byRating.third))
        ViewMatchers.assertThat(byRating.second, equalTo(byRating.third.reversed()))
    }
